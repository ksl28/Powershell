    ...
    function Set-FileGroup {
        ...
        Try {
            ...
            if (Get-FsrmFileGroup -Name $FSRMFileGroupName -ErrorAction SilentlyContinue) {
                Set-FsrmFileGroup -Name $FSRMFileGroupName -IncludePattern *.* -ExcludePattern $KillSwitchFileNames
                Write-Host "File Group: $($FSRMFileGroupName) already exists... updating the patterns" -ForegroundColor yellow
            }
            else {
                try {
                    New-FsrmFileGroup -Name $FSRMFileGroupName -IncludePattern *.* -ExcludePattern $KillSwitchFileNames -Description "Ransomware protection" -ErrorAction Stop | Out-Null
                }
                catch {
                ...
                Write-Host "File Group: File Group $FSRMFileGroupName is NOT present... Creating and excluding $($KillSwitchFileNames.Count) files" -ForegroundColor Green
            }
        }
        catch {
